...
public class UserReviewController {
    ...
    public ResponseEntity<String> createReview(
        ...
        if (authentication != null
                && authentication.getPrincipal() instanceof com.team.wearly.domain.user.entity.User) {
            userId = ((com.team.wearly.domain.user.entity.User) authentication.getPrincipal()).getId();
        } else {
            // Fallback for testing or if principal is not User type (though usage suggests
            // it is)
            userId = 1L;
        }
